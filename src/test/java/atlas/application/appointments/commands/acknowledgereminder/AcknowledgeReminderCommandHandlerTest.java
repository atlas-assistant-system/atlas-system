...
class AcknowledgeReminderCommandHandlerTest {
    ...
    private final AcknowledgeReminderCommandHandler handler =
        new AcknowledgeReminderCommandHandler(unitOfWork, Clock.fixed(NOW, ZoneOffset.UTC));
    ...
    @Test
    void shouldAcknowledgeTheReminderWhenItExists() {
        var appointment = withReminder();
        when(appointments.get(ID)).thenReturn(Optional.of(appointment));

        var result = handler.handle(new AcknowledgeReminderCommand(ID, REMINDER_ID));

        assertThat(result.isSuccess()).isTrue();
        assertThat(appointment.reminders().getFirst().isAcknowledged()).isTrue();
    }

    @Test
    void shouldStillSucceedWhenReminderIsAcknowledgedTwice() {
        var appointment = withReminder();
        when(appointments.get(ID)).thenReturn(Optional.of(appointment));
        handler.handle(new AcknowledgeReminderCommand(ID, REMINDER_ID));
